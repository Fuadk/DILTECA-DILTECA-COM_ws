...
import {   actfieldsAccountabilityActFieldsGrid , componentConfigDef } from '@modeldir/model';
...
'REC_SEQ' : new FormControl(dataItem.REC_SEQ  ,   Validators.required ) ,
...
@Component({
  selector: 'app-accountability-act-fields-grid',
  encapsulation: ViewEncapsulation.None,
  templateUrl: './accountability-act-fields-grid.component.html',
  styleUrls: ['./accountability-act-fields-grid.component.scss'
],
...
export class ActFieldsAccountabilityActFieldsGridGridComponent implements OnInit,OnDestroy {
...
public  isREC_SEQEnable : boolean = true; 
  ...
  private insertCMD = "INSERT_ACT_FIELDS";
  private updateCMD = "UPDATE_ACT_FIELDS";
  private deleteCMD =   "DELETE_ACT_FIELDS";
  private getCMD = "GET_ACT_FIELDS_QUERY";

  public  executeQueryresult:any;
  public title =  this.starServices.getNLS([],"accountability_ACT_FIELDS_GRID.actfieldsAccountabilityActFieldsGrid.component_title","Act Fields Accountability Act Fields Grid");
  ...
  public primarKeyReadOnlyArr = {isFILE_TYPEreadOnly : false , isFILE_NAMEreadOnly : false , isREC_SEQreadOnly : false};  
  public paramConfig;
...
public compSelector = 'app-accountability-act-fields-grid';
...
//Next part for filtering
  ...
  private gridInitialValues:any = new actfieldsAccountabilityActFieldsGrid();   
  ...
  public commonCallStarNotify(data:any) {
    ...
    let masterParams = {
      data: this.masterKeyArr
       ...
       let masterKeyArr = [data['FILE_TYPE'],data['FILE_NAME'],data['REC_SEQ']];
      let masterKeyNameArr = ['FILE_TYPE','FILE_NAME','REC_SEQ'];
      //for (let i = 0; i < masterKeyNameArr.length; i++) {
      //  componentConfig.[masterKeyNameArr[i]] = masterKeyArr[i];
      //}
...
this.lookupArrDef =[	{"statment":"SELECT CODE, CODETEXT_LANG , PARTCODE FROM SOM_TABS_CODES WHERE CODENAME = \"FILE_TYPE\"  and LANGUAGE_NAME = '" + this.userLang + "' order by CODETEXT_LANG ",
	...
	{"statment":"SELECT CODE, CODETEXT_LANG , PARTCODE FROM SOM_TABS_CODES WHERE CODENAME = \"LOGNAME\"  and LANGUAGE_NAME = '" + this.userLang + "' order by CODETEXT_LANG ",
...
var rec = this.lkpArrLOGNAME.find((x:any) => x.CODE === CODE);
return rec;
}
  ...
  window.print();
   ...
   public handleComponentConfig(ComponentConfig:any) {
      ...
      if (typeof ComponentConfig !== "undefined") {
         if (this.paramConfig.DEBUG_FLAG) console.log("ActFieldsAccountabilityActFieldsGridGrid ComponentConfig:", ComponentConfig);
...
async WHEN_VALIDATE_ITEM_REC_SEQ(formGroup) {

 this.FORM_TRIGGER_FAILURE = false ; 
 this.formGroup.controls['REC_SEQ'].setErrors({invalid: true}); 
 // Code goes here 
 ...
 this.formGroup.get('REC_SEQ').updateValueAndValidity();
 this.formGroup.updateValueAndValidity(); 
 }
...
async WHEN_VALIDATE_ITEM_LOGDATE(formGroup) {
 ...
 async onBlur_REC_SEQ() { 
  await this.WHEN_VALIDATE_ITEM_REC_SEQ(this.formGroup); if ( this.FORM_TRIGGER_FAILURE) return;  
 } 
