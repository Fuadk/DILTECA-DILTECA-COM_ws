...
*/
  ...
  async global_program(CHOICE_C, P_Form_Ver) {
    ...
    this.starServices.sessionParams["LANGUAGE_NAME"] = "";      	// Default system language set to english(EN)
    this.starServices.sessionParams["REP_LANG"] = "";      	// Default reports language set to english(EN)
    ...
    this.starServices.sessionParams["ROUTINE"] = "";
    ...
    if (this.starServices.sessionParams["ROUTINE"] == "") {
      let sqlStmt_Cur_Routine = "Select ROUTINE,MODULE "
        + "   from ROUTINES "
      ...
      if (this.FORM_TRIGGER_FAILURE == true) {
        ...
        if (ROUTINESInfo.length != 0) {
          GLOBAL_ROUTINE = ROUTINESInfo[0]["ROUTINE"];
          sModule = ROUTINESInfo[0]["MODULE"];
          this.starServices.sessionParams["ROUTINE"] = GLOBAL_ROUTINE;
    ...
    //   this.put_user_log (CHOICE_CD, this.starServices.sessionParams["ROUTINE"].toUpperCase(), P_Form_Ver )
    // }
  ...
  public global_program_org() {
   ...
   IS
    ...
    Cursor Cur_Routine is Select
       ROUTINE,MODULE from ROUTINES where PROGRAM = CHOICE_C;
  ...
  BEGIN
    ...
    DEFAULT_VALUE( '', 'GLOBAL.LANGUAGE_NAME');      	-- Default system language set to english(EN)
    DEFAULT_VALUE( '', 'GLOBAL.REP_LANG');      	-- Default reports language set to english(EN)
    ...
    if name_in('GLOBAL.ROUTINE') is null then
       Open Cur_Routine;
       Fetch Cur_Routine into GLOBAL_ROUTINE,sModule;
       If Cur_Routine%found then
          COPY(GLOBAL_ROUTINE, 'GLOBAL.ROUTINE');
  ...
  --  end if;
    ...
    -- Get system language form company profile
   if name_in('GLOBAL.LANGUAGE_NAME') is null then
    ...
    -- Get reports language form company profile
  ...
  --  if name_in('GLOBAL.SCREEN_LABEL') = 'Y' Then 
       ...
       if name_in('GLOBAL.USER_LOG') ='Y' then
         ...
         PUT_USER_LOG(choice_cd,UPPER(NAME_IN('GLOBAL.ROUTINE')),p_form_ver);
         
       end if;
  --  End If;
  
  
   -- check user authority to access the routine
    IF Authority.CheckAuthority(choice_c) != Authority.am_OK THEN
      DO_AUTH;
    END IF;
  ...
  --  if name_in('GLOBAL.FLEXI_FIELDS') is null then
   ...
   -- end if;
    if name_in('GLOBAL.FLEXI_FIELDS')  ='Y'  then
       SET_FLEXI_FIELD(name_In('GLOBAL.ROUTINE'));     
    end if;
    ...
    if name_in('GLOBAL.PROMPT_SETTING') = 'Y' then
       if name_in('GLOBAL.USER_LANGUAGE') is null then
          ...
          if userlang%notfound then
             ...
             copy(wuserlang.language,'GLOBAL.USER_LANGUAGE');
             copy(wuserlang.language,'GLOBAL.LANGUAGE_NAME');
          end if;
          close userlang;
       end if;
  ...
  *2
  ...
  cursor cprog is      
     SELECT PROGRAM,RTYPE,MODULE FROM ROUTINES WHERE routine = achoice;
  ...
  --  :global.routine := achoice;
  get_auth(achoice,'',b_auth);
  if b_auth = 0 then
    tabs_mess(1339,name_in('global.language'));
  elsif b_auth = 9 then
    tabs_mess(826,name_in('global.language'));
  else
    open cprog;
  ...
  --     open(b_program,ACTIVATE,NO_SESSION,
     ...
     close	c_sys;
  --      open c_adm;
  --      fetch c_adm into :global.language,:global.group,:global.retailer;
  ...
  if param = 'Y' then
      update routines_usage_log set udate = sysdate
            where username = wuser and routine = achoice
